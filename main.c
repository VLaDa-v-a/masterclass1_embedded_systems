...
void initGPIO() {
  ...
  // Настраиваем PA5 как выход (индикатор "система включена")
...
void checkTickCount() {
  if ((tickCount % 2000) == 0) {
    GPIOA->ODR ^= (1 << 5); // Мигаем встроенным LED - индикатор "система жива"
    printf("tickCount = %d!\n", tickCount++);
  }
}
...
int main(void) {
  ...
  printf("Access control system ready. Code: ****\n");

  GPIOA->ODR |= (1 << 5); // Включаем LED "питание"
