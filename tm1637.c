...
// Выводит 4 готовых сегмент-кода подряд, codes[0] - самый левый разряд
// (адрес 0xC0 в памяти TM1637 соответствует именно левому разряду).
void tm1637_display_raw(const uint8_t codes[4]) {
  tm1637_start();
  tm1637_write_byte(TM1637_CMD1);  // Автоинкремент адреса
  tm1637_stop();

  tm1637_start();
  tm1637_write_byte(TM1637_CMD2);  // Начальный (самый левый) адрес

  for (uint8_t i = 0; i < 4; i++) {
    tm1637_write_byte(codes[i]);
  }

  tm1637_stop();

  // Установка яркости / включение дисплея
  tm1637_start();
  tm1637_write_byte(TM1637_CMD3);
  tm1637_stop();
}

uint8_t tm1637_digit_code(uint8_t digit) {
  if (digit < sizeof(digit_codes)) {
